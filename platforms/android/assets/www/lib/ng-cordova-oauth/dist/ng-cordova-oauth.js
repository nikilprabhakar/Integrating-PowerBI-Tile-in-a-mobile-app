            ...
            azureAD: function(clientId, tenantId, resourceURL) {
                ...
                if(window.cordova) {
                    ...
                    if($cordovaOauthUtility.isInAppBrowserInstalled(cordovaMetadata) === true) {
                        ...
                        var browserRef = window.open('https://login.windows.net/' + tenantId + '/oauth2/authorize?response_type=code&client_id=' + clientId + '&redirect_uri=https://login.live.com/oauth20_desktop.srf', '_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
                        browserRef.addEventListener("loadstart", function(event) {
                            if((event.url).indexOf('https://login.live.com/oauth20_desktop.srf') === 0) {
                                var requestToken = (event.url).split("code=")[1];
                                //console.log("Request Token is:"+requestToken);
                                $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';

                                $http({method: "post", url: "https://login.windows.net/" + tenantId + "/oauth2/token", data: 
                                    "client_id=" + clientId + 
                                    "&code=" + requestToken + 
